...
k = 1.6*pi            #define fixed k 
x_vec = 0:0.01:8.0   #define temperature range of interest
...
f = 0.75   #define the f value (f=0.48 is very fine tuned to give good results)
...
#plot the moments 
plt_m = Plots.plot([0],[0],label="",xlabel=L"x",ylabel=L"x \cdot m_{\mathbf{k},r}(x)/m_{\mathbf{k},r}(0)",title="moments at k="*string(k/pi)*"π",legend=:topleft,xlim=(-0.2,7.5),size=(1.5*aps_width,aps_width))
...
using CairoMakie
x = 8.0                #define temperature (x=J/T)
a = 0.35                #define J2/J1
b = 0.0                #define J3/J1
c = 0.0                #define J4/J1

f = 0.2                 # a=0.0->f=0.48, a=0.2->f=0.4, a=0.4->f=0.58, a=0.45->f=0.6, a=0.7-> f=0.65, a=1.0->f=0.35 fits best for x in {0,2,4}
...
ax=Axis(fig[1,1],limits=(0,2,0,35),xlabel=L"k/ \pi",ylabel="Energy (meV)",title = "x = $x, J₂/J₁ = $a, f = $f",titlesize=20,xlabelsize=20,ylabelsize=20)
hm=CairoMakie.heatmap!(ax,[k[1]/π for k in k_vec],w_vec*13.79,JSkw_mat,colormap=:viridis,colorrange=(0.0,0.8),highclip=:white)
CairoMakie.Colorbar(fig[:, end+1], hm,size=8, label = L"S(\mathbf{k},\omega) (\text{meV}^{-1})")
#resize_to_layout!(fig);  
#SHOW PLOT
##############
display(fig)
save("Images/DSF_x_$(x)_J1_J2_$(a)_f_$(f)_4.png", fig)
