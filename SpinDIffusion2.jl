...
function comp_DS_Dyn_Gauß(x_vec::Vector{Float64},a::Float64,str::String;b::Float64=0.0,c::Float64=0.0,r_max=Int64=3,f::Float64=0.7)
    ...
    for x in x_vec
        x0 = x/sc
        δ_vec,extrap_params = get_delta_vec_ext_neu(x0,moments,r_max=r_max,f=f)
        push!(sig_x, 1/x0 * 1/Tχ(x0) * Richardson.extrapolate(w->sigma_neu(δ_vec,x0,w*sc,extrap_params),0.0;x0=1e-2)[1])
    end
...
function DS_a(x_vec::Vector{Float64},w::Float64,a_vec::Vector{Float64};b::Float64=0.0,c::Float64=0.0,r_max=Int64=3,f::Float64=0.7)
...
function DS_x_0(a_vec::Vector{Float64},str::String;b::Float64=0.0,c::Float64=0.0,r_max=Int64=3,f::Float64=0.7)
    plt5 = Plots.plot([0],[0],label="",xlabel=L"J2/J1",ylabel=L"D_S/J(x=0)",title="Spin Diffusion, $(lattice_type)",size=(1.5*aps_width,aps_width),legend=:bottomright)
    xs = Float64[]
    ys = Float64[]
    for a in a_vec
        c_iipDyn_mat_subst = load_ciipDyn_mat_subst(a,b=b,c=c)
        ...
        sig_x = Richardson.extrapolate(x->(1/x * 1/Tχ(x) * Richardson.extrapolate(w->sigma_neu(get_delta_vec_ext_neu(x,moments,r_max=r_max,f=f)[1],x,w*sc,get_delta_vec_ext_neu(x,moments,r_max=r_max,f=f)[2]),0.0;x0=1e-2)[1]),0.0;x0=1e-2)[1]
...
function DS_Bonca(x_vec::Vector{Float64},w::Float64;a::Float64=0.0,b::Float64=0.0,c::Float64=0.0,r_max=Int64=3,f::Float64=0.7)
    plt6 = Plots.plot([0],[0],label="",xlabel=L"T/J",ylabel=L"D_S/J",title="Spin Diffusion, $(lattice_type)",size=(1.5*aps_width,aps_width), legend=:topright, ylims=(0.4,0.7))
    c_iipDyn_mat_subst = load_ciipDyn_mat_subst(a,b=b,c=c)
    moments = get_moments_Gklim(c_iipDyn_mat_subst, hte_lattice, ex)
    c_iipEqualTime_mat = get_c_iipEqualTime_mat(c_iipDyn_mat_subst)
    Tχ = Polynomial([(-1)^n*sum(c_iipEqualTime_mat[i,1][n+1] for i in 1:hte_lattice.lattice.length) for n in 0:n_max])
    ...
    y_vec = [Diffusion_Gauß(Tχ, moments, x/sc) for x in x_vec]
    Plots.plot!(plt6, 1 ./ x_vec, y_vec, label="Dyn-HTE: Gauß Approximation")
    ...
    Plots.scatter!(plt6, bonca_x, bonca_y, label="[BoncaPRB1995]")

    Plots.plot!(plt6, 1 ./ x_vec, sqrt(pi/20) .* (1 .+ 21/20 .* x_vec), linestyle=:dash, label="[SokolPRL1993]")
...
####### main ######
x_vec = collect(0.01:0.5:4.0)
w_vec = collect(0.025:0.025:5.0)
a = 0.0
a_vec = [0.0,0.5,1.1,1.25,1.3,1.4,1.6,2.0,4.0]#,1.5,2.0,2.5,3.0,3.5,4.0]

# moments of σ(w) and R(w) are connected but shifted by one -> r_max=4 means we use 0th moment:m_{d,2}, 1st moment:m_{d,4}, 2nd moment m_{d,6}, 3rd moment m_{d,8}

#sigma_w_x(x_vec,w_vec,a,r_max=4)
#comp_DS_Dyn_Gauß(collect(0.001:0.01:1.0),a,"square",r_max=4)
#sigma_w_a(0.01,w_vec,a_vec,r_max=4)
#DS_x_0([0.0,0.2,0.5,1.1,1.25,1.3,1.4,1.5,1.6,2.0,2.5,4.0],"hypercubic",r_max=4) #[0.0,0.2,0.4,0.5,0.7,0.8,0.9,1.0,1.5,2.0,2.5,3.0,3.5,4.0]

#wlim = 0.001
#xlim = 0.01
#DS_Bonca(collect(0.1:0.01:1.0),wlim,r_max=4) #only for square lattice
#DS_a(collect(0.01:0.01:1.0),wlim,[0.0,0.2,0.4,0.5],r_max=3)
