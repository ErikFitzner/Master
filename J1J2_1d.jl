...
f = 0.51   #define the f value (f=0.48 is very fine tuned to give good results)
...
#plot the moments 
plt_m = Plots.plot([0],[0],label="",xlabel=L"x",ylabel=L"x \cdot m_{\mathbf{k},r}(x)/m_{\mathbf{k},r}(0)",title="moments at k="*string(k/pi)*"π",legend=:topleft,xlim=(-0.2,2.2),size=(1.5*aps_width,aps_width))
...
w_vec = collect(0:w_step_size:3.5)
...
ax=Axis(fig[1,1],limits=(0,2,0,3.5),xlabel=L"k/ \pi",ylabel=L"\omega/J_1",title = "x = $x, J₂/J₁ = $a, f = $f",titlesize=20,xlabelsize=20,ylabelsize=20)  #ylabel="Energy (meV)"
hm=CairoMakie.heatmap!(ax,[k[1]/π for k in k_vec],w_vec,JSkw_mat,colormap=:viridis,colorrange=(0.0,0.4),highclip=:white) #w_vec*13.79
CairoMakie.Colorbar(fig[:, end+1], hm,size=8, label = L"J_1 S(\mathbf{k},\omega)") #label = L"S(\mathbf{k},\omega) (\text{meV}^{-1})"
#resize_to_layout!(fig);  
#SHOW PLOT
##############
display(fig)
#save("Images/DSF_x_$(x)_J1_J2_$(a)_f_$(f).png", fig)
#save("Images/dimer_JSkw_a_$(a).png", fig)
...
if false
    ...
    ax=Axis(fig[1,1],xlabel=L"k/\pi",ylabel=L"J_1S(k,\omega)",xlabelsize=8,ylabelsize=8, title="$(lattice_type), w=$(wslice)");
    lines!(ax,[k[1]/π for k in k_vec],JSkw_mat[:,slice])
    #ax.xticks = ((kticks_positioins .- 1) ./ Nk,pathticks)
    display(fig)
    #save("Images/$(lattice_type)_wslice_$(wslice)_2.png",fig; px_per_unit=6.0)
end
